...
GNN-CeramicMap Train_Eval File
"""

import torch
from torch_geometric.loader import DataLoader
from src.egcn.dft_processor import DFTProcessor
from src.egcn.egcn import EnhancedGNNModel, GNNModel
import os
import numpy as np
...
class MaterialML:
    def _save_fig(self, fig, name: str) -> Path:
        ...
        out = self.plots_dir / f"{name}-{ts}.png"
    ...
    def __init__(self, dft_data_path, use_enhanced_model=True, focus_hard=False):
        ...
        self.plots_dir = self.root / "assets" / "plots"
    ...
    def train(self, train_loader, val_loader, hidden_dim=CONFIG["hidden_dim"], lr=CONFIG["lr"], epochs=CONFIG["epochs"]):
        ...
        print(f"Training plot saved under /assets")
...
if __name__ == "__main__":
    ...
    try:
        ...
        out_path = ml._save_fig(fig, "energy_parity")
        plt.show()
        print(f"Energy parity plot saved to {out_path}")
